...
#define WINDOW_TITLE    "【致我们永不熄灭的游戏开发梦想】Direct3D程序的核心框架"  //为窗口标题定义的宏  
...
{  
    ...
    wndClass.hIcon=(HICON)::LoadImage(NULL,"icon.ico",IMAGE_ICON,0,0,LR_DEFAULTSIZE|LR_LOADFROMFILE);  //本地加载自定义ico图标  
    ...
    //【3】窗口创建四步曲之三：正式创建窗口  
    HWND hwnd = CreateWindow( "ForTheDreamOfGameDevelop",WINDOW_TITLE,             //喜闻乐见的创建窗口函数CreateWindow  
    ...
    PlaySound("War3XMainScreen.wav", NULL, SND_FILENAME | SND_ASYNC|SND_LOOP); //循环播放背景音乐   
    MessageBox(hwnd, "DirectX，等着瞧吧，我们来降服你了~!", "浅墨的消息窗口", 0); //使用MessageBox函数，显示一个消息窗口  
    ...
    //【6】窗口类的注销  
    UnregisterClass("ForTheDreamOfGameDevelop", wndClass.hInstance);  //程序准备结束，注销窗口类  
    return 0;    
}  
